...
class OAuthValidationMiddleware:
    ...
    def __init__(self, get_response):
        ...
        self.provider_validators = {
            ...
            # Updated to match new provider name
            "linkedin_oauth2": self._validate_linkedin_token,
        }
        ...
        self.provider_refreshers = {
            ...
            # Updated to match new provider name
            "linkedin_oauth2": self._refresh_linkedin_token,
        }
    ...
    def _load_additional_providers(self):
        ...
        additional_providers = getattr(
            settings, "OAUTH_SESSION_VALIDATOR_PROVIDERS", {}
        )
        ...
        for provider_id, provider_config in additional_providers.items():
            if "validator" in provider_config:
                try:
                    ...
                    logger.warning(
                        f"Could not load validator for provider {provider_id}: {e}"
                    )
            ...
            if "refresher" in provider_config:
                try:
                    ...
                    logger.warning(
                        f"Could not load refresher for provider {provider_id}: {e}"
                    )
    ...
    def _apply_settings(self):
        ...
        self.VALIDATION_PROBABILITY = config.get(
            "VALIDATION_PROBABILITY", self.VALIDATION_PROBABILITY
        )
        self.VALIDATION_INTERVAL = config.get(
        ...
        self.SENSITIVE_PATHS = config.get(
            "SENSITIVE_PATHS",
            [
                "/password/change/",
                "/settings/",
        ...
        self.FINGERPRINT_SIMILARITY_THRESHOLD = config.get(
            "FINGERPRINT_SIMILARITY_THRESHOLD", 0.9
        )

        # Enhanced fingerprinting settings
        self.FINGERPRINT_COMPONENTS = config.get(
        ...
        self.FINGERPRINT_IP_MASK = config.get(
            "FINGERPRINT_IP_MASK", 24
        )  # Default to /24 subnet mask
        self.FINGERPRINT_USE_GEOLOCATION = config.get(
            "FINGERPRINT_USE_GEOLOCATION", False
        )  # Off by default
        self.FINGERPRINT_HASH_ALGORITHM = config.get(
            "FINGERPRINT_HASH_ALGORITHM", "sha256"
        )

        # Token refresh settings
        self.REFRESH_TOKEN_ENABLED = config.get("REFRESH_TOKEN_ENABLED", True)
        self.REFRESH_TOKEN_BEFORE_EXPIRY = config.get(
            "REFRESH_TOKEN_BEFORE_EXPIRY", 300
        )  # 5 minutes
        ...
        for failure_type, handler_path in handlers_config.items():
            try:
                ...
                if callable(handler):
                    ...
                    logger.warning(
                        f"Failure handler for {failure_type} is not callable"
                    )
            except (ImportError, AttributeError) as e:
                logger.warning(
                    f"Could not load failure handler for {failure_type}: {e}"
                )
    ...
    def _should_perform_security_checks(self, request):
        ...
        if isinstance(request.user, mock.Mock) or (
            hasattr(request.user, "socialaccount_set")
            and isinstance(request.user.socialaccount_set, mock.Mock)
        ):
            if hasattr(request.user, "socialaccount_set"):
                if hasattr(request.user.socialaccount_set, "exists"):
                    if callable(
                        getattr(request.user.socialaccount_set, "exists", None)
                    ):
                        has_social = True
    ...
    def _perform_security_checks(self, request):
        ...
        try:
            ...
            for account in social_accounts:
                ...
                if token.expires_at:
                    ...
                    if seconds_until_expiry < 0:
                        ...
                        and seconds_until_expiry < self.REFRESH_TOKEN_BEFORE_EXPIRY
                    ):
            ...
            validation_system_error.send(
                sender=self.__class__, request=request, user=user, error=e
            )
            return result
    ...
    def _handle_security_failure(self, request, result):
        ...
        if reason == "session_age_exceeded":
            messages.warning(
                request,
                _(
                    "Your session has exceeded the maximum allowed duration. "
            ...
            messages.warning(
                request,
                _(
                    "Your session appears to be compromised or accessed from a new device. "
                    "Please log in again for security reasons."
                ),
            )
            ...
            messages.warning(
                request,
                _(
                    "Your authorization with {provider} has been revoked or is no longer valid. "
                    "Please log in again."
            ...
            messages.warning(
                request,
                _(
                    "Your {provider} authorization has expired. " "Please log in again."
                ).format(provider=provider.title()),
            )
        else:
            messages.warning(
                ...
                if http_err.code == 401:
                    try:
                        ...
                        if error_data.get("error") == "invalid_token":
                            logger.warning(
                                "Google OAuth token has been revoked or is invalid"
                            )
                    except Exception:
                        pass
        ...
        try:
            ...
            with urllib.request.urlopen(request, timeout=10) as response:
                ...
                if new_token:
                    ...
                    logger.info(
                        f"Successfully refreshed Google token for user {account.user.id}"
                    )
                    return token
                else:
                    logger.warning(
                        "Google token refresh response did not contain a new token"
                    )
                    return None
        ...
        try:
            ...
            with urllib.request.urlopen(request, timeout=10) as response:
                ...
                if new_token:
                    ...
                    logger.info(
                        f"Successfully refreshed Microsoft token for user {account.user.id}"
                    )
                    return token
                else:
                    logger.warning(
                        "Microsoft token refresh response did not contain a new token"
                    )
                    return None
        ...
        try:
            ...
            with urllib.request.urlopen(request, timeout=10) as response:
                ...
                if new_token:
                    ...
                    logger.info(
                        f"Successfully refreshed LinkedIn token for user {account.user.id}"
                    )
                    return token
                else:
                    logger.warning(
                        "LinkedIn token refresh response did not contain a new token"
                    )
                    return None
