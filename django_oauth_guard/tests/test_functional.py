...
class FunctionalTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.account = SocialAccount.objects.create(
            user=self.user, provider="google", uid="test-uid"
        )
    ...
    @mock.patch(
        "django_oauth_guard.middleware.OAuthValidationMiddleware._validate_google_token"
    )
    ...
    def test_inactivity_timeout_behavior(self, mock_validate):
        ...
        messages = list(get_messages(response.wsgi_request))
        self.assertTrue(
            any("timed out due to inactivity" in str(msg) for msg in messages)
        )
...
class MultiProviderFunctionalTestCase(TestCase):
    ...
    def setUp(self):
        ...
        # Create a test user
        self.user = User.objects.create_user(
            username="multiprovider", email="multi@example.com", password="testpassword"
        )
        ...
        for provider in self.providers:
            ...
            account = SocialAccount.objects.create(
                user=self.user, provider=provider, uid=f"{provider}-uid"
            )
    ...
    @mock.patch(
        "django_oauth_guard.middleware.OAuthValidationMiddleware._validate_facebook_token"
    )
    @mock.patch(
        "django_oauth_guard.middleware.OAuthValidationMiddleware._validate_github_token"
    )
    ...
    def test_one_invalid_provider_fails_all(
        self, mock_github, mock_facebook, mock_google
    ):
