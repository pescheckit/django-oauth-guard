...
class TokenRefreshTestCase(ProviderSetupMixin, RequestMixin, TestCase):
    ...
    def test_google_token_refresh(self):
        ...
        token = self.create_social_token(
            ...
            - timedelta(hours=1),  # Expired token, using timezone-aware datetime
        )
        # Add a refresh token
    ...
    def test_microsoft_token_refresh(self):
        ...
        with mock.patch("urllib.request.urlopen") as mock_urlopen:
            ...
            # Also mock token validation to return True for the new token
            with mock.patch.object(
                self.middleware, "_validate_microsoft_token", return_value=True
            ):
                # Process the request
                response = self.middleware(request)
    ...
    def test_linkedin_token_refresh(self):
        ...
        with mock.patch("urllib.request.urlopen") as mock_urlopen:
            ...
            # Also mock token validation to return True for the new token
            with mock.patch.object(
                self.middleware, "_validate_linkedin_token", return_value=True
            ):
                # Process the request
                response = self.middleware(request)
    ...
    def test_facebook_token_refresh_not_supported(self):
        ...
        with mock.patch.object(
            ...
            with mock.patch.object(
                self.middleware, "_refresh_facebook_token", return_value=None
            ):
    ...
    def test_github_token_refresh_not_supported(self):
        ...
        with mock.patch.object(
            self.middleware, "_validate_github_token", return_value=True
        ):
            # Mock refresh to return None as GitHub doesn't support refresh
            with mock.patch.object(
                self.middleware, "_refresh_github_token", return_value=None
            ):
                # Mock logout to track calls
                with mock.patch("django.contrib.auth.logout") as mock_logout:
                    # Process the request
    ...
    def test_proactive_token_refresh(self):
        ...
        token = self.create_social_token(
            account, token="soon-to-expire", expires_at=soon_expiry
        )
        token.token_secret = "refresh-token"
        token.save()
...
class EnhancedFingerprintingTestCase(RequestMixin, TestCase):
    ...
    def test_custom_fingerprint_components(self):
        ...
        try:
            ...
            self.middleware.FINGERPRINT_COMPONENTS = [
                "HTTP_USER_AGENT",
            ...
            request2.META["REMOTE_ADDR"] = (
                "192.168.5.5"  # Different but should be ignored
            )
    ...
    def test_custom_hash_algorithm(self):
        ...
        try:
            ...
            self.middleware.FINGERPRINT_HASH_ALGORITHM = (
                "md5"  # Not recommended for security, just for testing
            )
...
class SignalIntegrationTestCase(ProviderSetupMixin, RequestMixin, TestCase):
    ...
    def test_token_validation_failed_signal(self):
        ...
        try:
            ...
            with mock.patch.object(
                self.middleware, "_validate_google_token", return_value=False
            ):
    ...
    def test_token_refreshed_signal(self):
        ...
        try:
            ...
            with mock.patch.object(
                self.middleware, "_try_refresh_token"
            ) as mock_refresh:
    ...
    def test_session_fingerprint_mismatch_signal(self):
        ...
        def handle_fingerprint_mismatch(
            sender, request, user, similarity, threshold, **kwargs
        ):
        ...
        try:
            ...
            with mock.patch.object(
                ...
                return_value={"valid": False, "similarity": 0.5, "threshold": 0.9},
            ):
